...
var delay_file = "user://delay.save"
...
var isExcited = false
var bounceX = 1.5
var excitedBounceRate = 6
var maxExcitedBounceRate = 6

var isNervous = false
var nervousBounceX = 3
var nervousBounceRate = 12
var maxNervousBounceRate = 12

var wasVibing = false
var wasShaking = false
var wasShakingMore = false
var wasBouncy = false
var wasExcited = false
var wasNervous = false
...
func vibing(isVibing):
	if(isVibing):
		if(displacementX > vibeXmax and changeCount == 0):
			...
			change = true
			changeCount = 55
		if(displacementX < vibeXmin and changeCount == 0):
			vibeRight = true
			vibeLeft = false
			change = true
			changeCount = 55
...
#			print(rand)
		...
		if(vibeRight):
			if(changeCount > 0):
				displacementAmount = .3 + rand/2
				changeCount = changeCount - 1
			else: 
				if(abs(displacementX) > abs(vibeXmax)*.5):
					displacementAmount += rand/12 - (abs(displacementX)/abs(vibeXmax))*.2
				else:
					displacementAmount += rand/12
		if(vibeLeft):
			if(changeCount > 0):
				...
				if(abs(displacementX) > abs(vibeXmax)*.5):
					...
					displacementAmount += rand/12
		if(displacementY > vibeYmax and changeCountY == 0):
			vibeUp = false
			vibeDown = true
			changeY = true
			changeCountY = 55
		if(displacementY < vibeYmin and changeCountY == 0):
			vibeUp = true
			vibeDown = false
			changeY = true
			changeCountY = 55
...
#			print(rand)
		...
		if(vibeUp):
			if(changeCountY > 0):
				displacementAmountY = .3 + rand/12
				changeCountY = changeCountY - 1
			else: 
				if(abs(displacementY) > abs(vibeYmax)*.5):
					displacementAmountY += rand/12 - (abs(displacementY)/abs(vibeYmax))*.2
				else:
					displacementAmountY += rand/12
		if(vibeDown):
			if(changeCountY > 0):
				displacementAmountY = -.3 + rand/2
				changeCountY = changeCountY - 1
			else: 
				if(abs(displacementY) > abs(vibeYmax)*.5):
					displacementAmountY += rand/12 - (abs(displacementY)/abs(vibeYmax))*.2
		...
		if (displacementAmountY > 0.3):
			displacementAmountY = 0.3
...
func shaking(isShaking):
	if(isShaking):
		if(count == 0):
			displacementX = (rng.randf() - .5)*6
			displacementY = (rng.randf() - .5)*6
		else:
			if(count == 1):
				count = 0 
			else:
...
func shakingMore(isShakingMore):
	if(isShakingMore):
		displacementX = (rng.randf() - .5)*12
		displacementY = (rng.randf() - .5)*12
...
func bouncy(isBouncy):
	if(isBouncy):
		if(bounceRate == maxBounceRate * -1):
			bounceRate = maxBounceRate
		else:
			bounceRate -= 0.25
...
func excited(isExcited):
	if(isExcited):
		if(excitedBounceRate == maxExcitedBounceRate * -1):
			excitedBounceRate = maxExcitedBounceRate
			bounceX *= -1
		else:
			excitedBounceRate -= 1
...
func nervous(isNervous):
	if(isNervous):
		if(nervousBounceRate == maxNervousBounceRate * -1):
			nervousBounceRate = maxNervousBounceRate
			bounceX *= -1
		else:
			nervousBounceRate -= 3
...
#		movie_save._write_begin(DisplayServer.window_get_size(),120,path_video)
		...
		print("File audio not exist!!! ",music_file )
...
#		dir = str_to_var(file_D.get_line())
...
#				talk_png_D = animated_texture
	...
	if FileAccess.file_exists(talking_file_E):
		...
		#dir = str_to_var(file_E.get_line())
...
#		#dir = str_to_var(file_F.get_line())
...
#		#dir = str_to_var(file_H.get_line())
...
#		#dir = str_to_var(file_I.get_line())
...
#		dir = str_to_var(file_J.get_as_text())#dir = str_to_var(file_J.get_line())
...
#				talk_png_J = animated_texture
	...
	if FileAccess.file_exists(silence_file_A):
		...
		dir = str_to_var(file_A.get_as_text())
#		dir = str_to_var(file_A.get_line())
...
#				silence_png_A = animated_texture
	...
	if FileAccess.file_exists(silence_file_B):
		...
		dir = str_to_var(file_B.get_as_text())
#		dir = str_to_var(file_B.get_line())
...
#				silence_png_B = animated_texture
	...
	if FileAccess.file_exists(silence_file_C):
		...
		dir = str_to_var(file_C.get_as_text())
#		dir = str_to_var(file_C.get_line())
...
#			$CanvasLayer/FileDialog.hide()
		...
		if files[0].get_extension() == "JPEG" or files[0].get_extension() == "jpeg" or files[0].get_extension() == "JPG" or files[0].get_extension() == "jpg" or files[0].get_extension() == "PNG" or files[0].get_extension() == "png":
			...
			if files.size() > 1:
				...
				print("Silence 1 image - not GIF ",files[0])
				image.load(files[0])
#				image_texture.create_from_image(image)
				image_texture.set_image(image)
...
#
		...
		print("Silence blind status")
		if files[0].get_extension() == "JPEG" or files[0].get_extension() == "jpeg" or files[0].get_extension() == "JPG" or files[0].get_extension() == "jpg" or files[0].get_extension() == "PNG" or files[0].get_extension() == "png":
			print("Silence right images")
			var image = Image.new()
			var image_texture = ImageTexture.new()
			image.load(files[0])
			image_texture.create_from_image(image)
...
func _on_RemoveBtn_pressed():
	...
	#pROP
	elif is_preset_A and is_filedialog_prop:
		prop_png_A = null
		#var file = FileAccess.new()
		var file =FileAccess.open(prop_file_A, FileAccess.WRITE)
		#file.close()
